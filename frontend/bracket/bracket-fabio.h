...
//Recebe uma expressao lambda, um caracter do bracket e um intervalo de inicio e fim
//o intervalo representa o combinador a ser avaliado
//Retorna 1 se dentro deste combinador existe uma variável do bracket, senão retorna 0
int avaliarArgumento(char* lambda, char bracket, int inicio, int fim) {
    char bracketLowerCase = tolower(bracket);
    int i;
    for (i = inicio; i <= fim ; i++) {
...
//Retorna 1 se o caracter eh um combinador da logica combinatorial e 0 senao eh
int isCombinador(char caracter) {
    char combinadores[10] = "KSIBCDEFY\0";
    int j;
    for (j = 0; j < 10; j++) {
        if(caracter == combinadores[j])
            return 1;
    }
    return 0;
}
...
int avaliarSeExpressaoEstaReduzida(char* lambda) {
    int i;
    for (i = 0; i < strlen(lambda); i++) {
        //Se for letra maiuscula e nao for um combinador, entao a expressao ainda nao esta reduzida
        if(isupper(lambda[i])) {
            if(!isCombinador(lambda[i])) {
                return 0;
            }
        }
    }
...
//Xa -> Ka, se a constante
//Xabcd -> K(abcd)
void converterK(char* lambda) {
    inserirParenteses(lambda,1, strlen(lambda));
    lambda[0] = 'K';

    //TODO caso a quantidade de argumentos seja 1, nao eh necessario parensesis
}
...
void bracketCasoBase(char* lambda) {
    ...
    /*
     *  se tamanho da string == 2
     *      se string argumento constante (Xa)
     *          reduzir K
     *      se string argumento nao for constante (Xx)
     *          reduzir I
     *
     *  se tamanho da string > 2
     *      se numero de argumentos > 2
     *          currificar
     *
     *      se argumento 1 e 2 constantes
     *
     *
     *
     * XZ+xz
    ...
    */
    ...
    int lenght = strlen(lambda);
    if(lenght == 2) {
        char bracket = tolower(lambda[0]);
        char arg = lambda[1];
        if(bracket == arg) {
            converterI(lambda);
            return;
...
//    int argumentosConstantes = checarSeArgumentosSaoConstantes(lambda);
...
void bracket(char* lambda) {
    ...
    if(avaliarSeExpressaoEstaReduzida(lambda)) {
        return;
    }
    ...
    int penultimoBracket = ultimoBracket-1;
    // Index do bracket anterior ao mais interno
